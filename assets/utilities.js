  ...
  loadNextPage: function loadNextPage(url, $button) {
    ...
    $.ajax({
      ...
      success: function success(data) {
        ...
        var thumbnails = $(data).find(_this.defaults.grid).html();
        var loadMoreButtonUrl = $(data).find('[data-load-more]').attr('href');
        $('[data-load-more]').attr('href', loadMoreButtonUrl);
        if($('.collection__main [data-load-more--grid] .item-chat').length > 0){
          $(thumbnails).insertBefore('[data-load-more--grid] .item-chat');
        }else{
