...
Shopify.theme.jsAjaxCart = {
  init: function init($section) {
    ...
    $(document).on('click', '.ajax-submit', function (e) {
      if($('body.customer-wholesalse').length && $('body.collection').length && $(this).attr('data-url-detail') != ''){
          location.href = $(this).attr('data-url-detail');
      }else if($('body.customer-wholesalse').length && $('body.product').length){
          e.preventDefault();
  ...
  addToCart: function addToCart($addToCartForm) {
    ...
    $.ajax({
      ...
      success: function success(product) {
        if($('body.customer-wholesalse').length && $('body.product').length){
          location.href = '/cart';
        }else{
          var $el = $('[data-ajax-cart-trigger]');
